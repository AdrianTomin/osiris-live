...
function isExchangeOpen(ex: Exchange): boolean {
    try {
        const now = new Date();
...
function riskLevelFor(base: number): CountryRisk['risk_level'] {
...
export async function fetchCountryRisk(): Promise<{
    ...
    try {
        const exchangeStatus: ExchangeStatus[] = EXCHANGES.map(ex => ({
        ...
        const countries: CountryRisk[] = Object.entries(RISK_FACTORS).map(([code, data]) => ({
            code,
            risk_score: Math.min(100, data.base + (quakeRisks[code] || 0)),
            risk_level: riskLevelFor(data.base),
            tags: data.tags,
        ...
        return { countries: [], exchanges: [], open_exchanges: 0, total_exchanges: 0, timestamp: new Date().toISOString(), error: 'Failed' };
    }
}