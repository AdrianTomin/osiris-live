...
// Shape of a single aircraft record as returned by adsb.lol
...
async function fetchRegion(region: Region): Promise<RawAircraft[]> {
    try {
...
function classifyFlight(f: RawAircraft): ClassifiedFlight | null {
    ...
    if (
        ...
        (f.flight || '').match(/^(RCH|KING|DUKE|EVAC|JAKE|REACH|CONVOY)\d/i)
    ) {
        category = 'military';
        ...
        !airlineCode &&
        modelUpper &&
        !['A319','A320','A321','A332','A333','A339','A343','A359','A388','B737','B738','B739','B38M','B39M','B752','B753','B763','B764','B772','B77L','B77W','B788','B789','B78X','E170','E175','E190','E195','CRJ7','CRJ9','AT43','AT72','DH8D'].includes(modelUpper)
    ) {
        category = 'private';
    }
...
function aggregateJamming(points: JammingPoint[], threshold: number): JammingZone[] {
...
// In-memory cache — this now runs in a single long-lived desktop process,
// so unlike the old serverless multi-isolate caveat, this cache is fully shared.
let cachedData: FlightsResult | null = null;
let lastFetchTime = 0;
const CACHE_TTL = 45000;
let fetchPromise: Promise<FlightsResult> | null = null;

export async function fetchFlights(): Promise<FlightsResult> {
    console.log('[DEBUG] fetchFlights called');
    const now = Date.now();
    ...
    fetchPromise = (async (): Promise<FlightsResult> => {
