...
interface RawArticle extends RSSItem {
    source: string;
}
...
// generateAssessment() REMOVED (kept from original) — was fabricating fake
// probabilistic threat percentages using Math.random(). See Issue #116.
...
export async function fetchNews(): Promise<{
    ...
    try {
        const feedPromises = Object.entries(FEEDS).map(async ([source, url]): Promise<RawArticle[]> => {
            try {
        ...
        return { news: [], total: 0, timestamp: new Date().toISOString(), error: 'Failed to fetch news' };
    }
}