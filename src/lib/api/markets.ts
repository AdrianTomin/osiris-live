...
async function fetchYahooV6(symbol: string): Promise<Quote | null> {
    try {
        ...
        if (!res.ok) return null;
        const data: YahooQuoteResult = await res.json();
...
export async function fetchMarkets(): Promise<MarketsResult> {
    try {
        ...
        return { stocks, oil, commodities, crypto, indices, timestamp: new Date().toISOString() };
    } catch (error) {
        console.error('Markets fetch error:', error);
        return { stocks: {}, oil: {}, commodities: {}, crypto: {}, indices: {}, timestamp: new Date().toISOString(), error: 'Failed' };
    }
}