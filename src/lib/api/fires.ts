...
export async function fetchFires(): Promise<{
    fires: FireEvent[];
    total: number;
    source: string;
    timestamp: string;
    ...
    try {
        ...
        try {
            ...
            if (volcRes.ok) {
                ...
                const volcanoes: FireEvent[] = (volcData.events || []).map((e: any) => {
        ...
        return { fires: [], total: 0, source: 'Unknown', timestamp: new Date().toISOString(), error: 'Failed to fetch fire data' };
    }
}

function parseCSV(csv: string): FireEvent[] {
